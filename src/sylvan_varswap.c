...
int is_node_dependant_on(mtbddnode_t node, BDDVAR var);
...
#else
VOID_TASK_DECL_3(sylvan_varswap_p0, uint32_t, uint64_t, uint64_t);
/*!
...
*/
#define sylvan_varswap_p0(var) CALL(sylvan_varswap_p0, var, 2, nodes->table_size)
#endif
...
TASK_IMPL_1(varswap_t, sylvan_varswap, uint32_t, pos)
{
    ...
    if (interact_test(levels, pos, pos + 1) == 1) {
...
#if !SYLVAN_USE_LINEAR_PROBING
VOID_TASK_IMPL_3(sylvan_varswap_p0,
...
//                    marked++;
             ...
             if (is_node_dependant_on(node, var)) {
                // mark for phase 2
//                mtbddnode_setflag(node, 1);
//                marked++;
                swap_node(first);
...
void swap_node(size_t index)
{
    ...
    if (!mtbdd_isleaf(f0)) {
        ...
        if (mtbddnode_getvariable(n0) == var+1) {
    ...
    if (!mtbdd_isleaf(f1)) {
        ...
        if (mtbddnode_getvariable(n1) == var+1) {
    ...
    BDDVAR f_v = var;
    BDDVAR f0_v = mtbdd_getvar(f0);
    BDDVAR f1_v = mtbdd_getvar(f1);
    BDDVAR f00_v = mtbdd_getvar(f00);
    BDDVAR f01_v = mtbdd_getvar(f01);
    BDDVAR f10_v = mtbdd_getvar(f10);
    BDDVAR f11_v = mtbdd_getvar(f11);
    (void)f_v; (void)f0_v; (void)f1_v; (void)f00_v; (void)f01_v; (void)f10_v; (void)f11_v; // prevent compiler warning
