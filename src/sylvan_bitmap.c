 ...
 */
static inline uint64_t get_first_msb_one_bit_pos(uint64_t word, size_t word_idx)
{
 ...
 */
static inline uint64_t get_first_lsb_one_bit_pos(uint64_t word,  size_t word_idx)
{
...
size_t bitmap_next(bitmap_t *bitmap, size_t size, size_t pos)
{
    ...
    uint64_t word = bitmap->container[word_idx] & BIT_FWD_ITER_MASK(pos);
    if (word) {
...
size_t bitmap_prev(bitmap_t *bitmap, size_t pos)
{
    ...
    uint64_t word = bitmap->container[word_idx] & BIT_BCK_ITER_MASK(pos);
    if (word) {
...
size_t atomic_bitmap_next(atomic_bitmap_t *bitmap, size_t pos)
{
    ...
    uint64_t word = atomic_load_explicit(ptr, memory_order_relaxed) & BIT_FWD_ITER_MASK(pos);
    if (word) {
        // there exist some successor 1 bit in the word, thus return the pos directly
        return get_first_msb_one_bit_pos(word, word_idx);
    } else {
...
size_t atomic_bitmap_prev(atomic_bitmap_t *bitmap, size_t pos)
{
    ...
    _Atomic(uint64_t) *ptr = bitmap->container + word_idx;
    // check whether there are still any predecessor 1-bits in the current word
    uint64_t word = atomic_load_explicit(ptr, memory_order_acquire) & BIT_BCK_ITER_MASK(pos);
    if (word) {
        // there exist some predecessor 1 bit in the word, thus return the pos directly
        return get_first_lsb_one_bit_pos(word, word_idx);
    } else {
